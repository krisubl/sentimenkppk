...
with st.expander('Enter Kata Kunci'):
    ...
    if st.button("RUN", key="runnin") :
        ...
        command = f'npx tweet-harvest@latest -o "$filename" -s "$search_keyword" -l 600 --token "$MY_API_TOKEN"'
...
with st.expander('Analisis EXCEL'):
    upl = st.file_uploader('Upload file dengan format xlsx')
